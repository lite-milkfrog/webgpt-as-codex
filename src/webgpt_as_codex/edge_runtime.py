...
from .edge import auth_proxy_binary, start_funnel, stop_funnel, tailscale_dns_name, wait_http
...
def public_base_url(public_port: int = PUBLIC_PORT) -> str:
    return f"https://{tailscale_dns_name()}:{public_port}"
...
def _write_edge_state(*, status: str, public_base: str | None, detail: str | None = None) -> None:
    atomic_write_json(
        _edge_state_path(),
        {
            "status": status,
...
def run_edge_runtime() -> int:
    ...
    try:
        ...
        wait_http(
            f"http://127.0.0.1:{AUTH_PORT}/.well-known/oauth-protected-resource",
            expected={200},
        )
        ...
        _write_edge_state(status="running", public_base=public_base)
