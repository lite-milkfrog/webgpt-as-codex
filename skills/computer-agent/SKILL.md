---
name: computer-agent
description: 在多 MCP 本地/远程电脑环境中为 Agent 选择正确工具并安全完成跨代码、文件、浏览器和 Windows GUI 任务。涉及 Serena、Coding Tools、Desktop Commander、Playwright、Windows-MCP 或它们的组合时使用；纯知识问答、无需电脑操作的写作与普通聊天不应触发。
metadata:
  version: 1.2.0
  portability: local-first-gpt-web-ready
  secrets-policy: no-secrets-in-skill
---

# Computer Agent

这是一个“路由 + 权限 + 工作流 + 校验”Skill，不是 MCP 手册集合。目标是让模型直接调用最合适的真实 MCP，减少重复工具、错误路由、危险操作和 GUI 误点。

## 发行 / 本机一致性

- \`skills/computer-agent/\` 是发行版 canonical portable core。
- 本机 \`.skills/computer-agent/\` 使用同一个 \`1.2.0\` portable core，只额外保留 \`environment.local.md\`、\`MCP-SKILLS-INVENTORY.*\`、\`state/\` 等 machine-local overlay。
- portable 文件不允许“本机先长、发行版以后再补”或反向漂移；使用 \`scripts/sync_computer_agent_skill.py --check\` 验证。
- WebGPT-as-Codex 产品专项 profile 保留在 \`skills/webgpt-as-codex/\`，与本 core 同版本发布；它的完整 Experience Ledger 同步进入本 core 的 \`experience-ledger.md\`，不能缩减。
- MCP 专项经验继续放 \`mcp-guides/\`；本地端口、路径、账户态和 transient health 仍只放 machine-local overlay，不进入 portable release。
...
25. **Prompt 的位置和文件必须可定位**：每一棒 prompt 必须显式写出 Computer Agent Skill 根、当前 Stage prompt、next prompt、SoT 根、产品源码根、tests 根与关键 MCP locator。对于 JARVIS 当前 workspace，Skill 实际根在 Coding Tools workspace 的 `.skills/computer-agent/`，不在 `Jarvis-dev/.skills/`；next worker 不得因为 worktree 相对路径找不到 Skill 就跳过 Skill bootstrap。
