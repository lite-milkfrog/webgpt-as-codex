...
The Windows launcher is a control surface, not a process parent contract. It starts/ensures the Manager and repository-managed runtimes, optionally opens the loopback Manager URL, then may exit while those runtimes continue. Desktop and autostart integration use transparent `.cmd` files in the user Desktop/Startup shell folders. Installation is reversible and ownership-checked: an existing unmanaged file is neither overwritten nor deleted, and no password/token/cookie/private URL is embedded.
